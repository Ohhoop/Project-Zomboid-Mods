...
local function applyVisualToPlayer(player, data, visualItemTypes)
    if not player or not data or not data.visual or isMultiplayer() then
        return
    end
...
local function applyClothingToPlayer(player, data, options)
    ...
    scheduler.scheduleAfterTicks(taskKey, 10, function()
...
function QuickRestartApply.applyLoadedCharacter(player, data, options)
    ...
    applyVisualToPlayer(player, data, options.visualItemTypes or {})
