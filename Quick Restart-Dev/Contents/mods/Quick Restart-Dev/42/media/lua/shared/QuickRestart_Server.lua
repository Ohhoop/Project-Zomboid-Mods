...
local function validateSnapshotRecord(profileKey, snapshot)
    local valid, reason = QuickRestartValidate.validateLegacyCharacterData(snapshot)
    if valid then
        return true, nil
    end
...
local function normalizeAndValidateSnapshot(profileKey, snapshot)
    local normalizedSnapshot = QuickRestartValidate.normalizeLegacyCharacterData(snapshot)
...
local function getProfileRecord(profileKey)
    ...
    local snapshot = QuickRestartLegacyCodec.readDataFromFile(fileName)
...
persistSnapshot = function(profileKey, snapshot)
    ...
    local fileName = QuickRestartProfileKey.getServerSnapshotFileName(profileKey)
    local stored = QuickRestartLegacyCodec.writeDataToFile(snapshot, fileName, snapshot.sandbox)
    if not stored then
...
local function onClientCommand(module, command, player, args)
    ...
    if command == COMMANDS.APPLY_SKILLS then
        cleanupExpiredRestartGrants()
        ...
        if not grant then
            ...
            sendServerCommand(player, MODULE, COMMANDS.APPLY_SKILLS_RETRY, QuickRestartProtocol.buildApplySkillsResponse({
                profileKey = profileKey,
        ...
        if not record or not record.snapshot then
            sendServerCommand(player, MODULE, COMMANDS.APPLY_SKILLS_DENIED, QuickRestartProtocol.buildApplySkillsResponse({
                profileKey = profileKey,
        ...
        QuickRestartServerState.restartGrantsById[grantId] = nil
        sendServerCommand(player, MODULE, COMMANDS.APPLY_SKILLS_ACK, QuickRestartProtocol.buildApplySkillsResponse({
            profileKey = profileKey,
        }))
    end
end
